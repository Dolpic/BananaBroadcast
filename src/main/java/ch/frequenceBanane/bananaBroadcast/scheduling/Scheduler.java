...
/**
 * The Scheduler is able to create a list of musics from a database and retrieve
 * the musics from it
 * 
 * @author Corentin
...
public class Scheduler {
	 ...
	 * @return A list of categories corresponding to the planning of the given date
	 *         + hour
	 */
	...
	/**
	 * Set the categories associated to a specific date and hour in the default
	 * planning
	 * 
	 * @param date       The day to which the planning must be set
	 * @param hour       The hour to which the planning must be set
		...
		try {
			for (; hoursToFill > 0; hoursToFill--) {
				...
				if (database.getScheduledAt(curDate, curHours).isEmpty()) {
					...
					database.getRandomMusicFromCategorie(planning, DEFAULT_NUMBER_OF_MUSIC_PER_HOUR).forEach(music -> {
						try {
