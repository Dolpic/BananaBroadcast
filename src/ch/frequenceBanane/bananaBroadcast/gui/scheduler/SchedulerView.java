...
public class SchedulerView {
	...
	@FXML private GridPane plannerGrid;
	@FXML private GridPane plannerGridDefault;
	
	@FXML private Label plannerLabel;
	@FXML private Label plannerLabelDefault;
	
	@FXML private Label rightTitleLabel;
	@FXML private Label rightTitleLabelDefault;
	
	@FXML private Button weekBackButton;
	@FXML private Button weekForwardButton;
	
	@FXML private Button saveButton;
	@FXML private Button saveButtonDefault;
	
	@FXML private VBox categoriesList;
	@FXML private VBox categoriesListDefault;
					...
					for(int hour=0; hour<24; hour++) {
						...
						if(gridPane[day][hour] == pane) {
							if(isDefaultPlanning)
								...
								currentCategoriesSelection = scheduler.getPlanningOf(selectedWeekMonday.plusDays(day), hour);
	...
	private void setButtonsEvents() {
		...
		saveButton.setOnMouseClicked(new EventHandler<MouseEvent>() {
			@Override public void handle(MouseEvent e) {
				scheduler.setPlanningOf(selectedWeekMonday.plusDays(selectedPaneDay), selectedPaneHour, currentCategoriesSelection);
				updatePlanner();
			}
		});
	}
